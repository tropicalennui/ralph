...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  try {
    ...
    if (name === "jira_get_issue") {
      result = await jira("GET", `/issue/${args.key}?fields=summary,status,assignee,description,issuetype,priority`);
      result = {
        key: result.key,
      ...
      result = await jira("POST", "/issue", body);
      ...
      result = await jira("PUT", `/issue/${args.key}`, { fields });

    } else if (name === "jira_transition_issue") {
      const { transitions } = await jira("GET", `/issue/${args.key}/transitions`);
      ...
      result = await jira("POST", `/issue/${args.key}/transitions`, { transition: { id: match.id } });

    } else if (name === "jira_assign_issue") {
      result = await jira("PUT", `/issue/${args.key}/assignee`, { accountId: args.accountId });

    } else if (name === "jira_add_comment") {
      result = await jira("POST", `/issue/${args.key}/comment`, {
        body: {
          type: "doc",
          version: 1,
