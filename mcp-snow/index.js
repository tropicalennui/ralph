...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  try {
    ...
    if (name === "snow_query_records") {
      ...
      result = await snow("GET", `/table/${args.table}?${params}`);

    } else if (name === "snow_get_record") {
      const params = new URLSearchParams();
      if (args.fields) params.set("sysparm_fields", args.fields);
      params.set("sysparm_display_value", "false");
      result = await snow("GET", `/table/${args.table}/${args.sys_id}?${params}`);

    } else if (name === "snow_create_record") {
      result = await snow("POST", `/table/${args.table}`, args.fields);

    } else if (name === "snow_update_record") {
      result = await snow("PATCH", `/table/${args.table}/${args.sys_id}`, args.fields);

    } else if (name === "snow_delete_record") {
      result = await snow("DELETE", `/table/${args.table}/${args.sys_id}`);
      ...
      result = await snow("GET", `/table/sys_dictionary?${params}`);
